...
db.serialize(() => {
    ...
    // Update live DB About Us content colors to match logo colors
    db.run("UPDATE settings SET value = REPLACE(value, 'class=\"text-secondary font-bold\"', 'class=\"text-[#FFF200] font-bold\"') WHERE key = 'about_us_content'");
    db.run("UPDATE settings SET value = REPLACE(value, '<span class=\"text-white font-bold\">Teen Patti Stars</span>', '<span class=\"text-white font-bold\">Teen Patti <span class=\"text-[#FFF200]\">Stars</span></span>') WHERE key = 'about_us_content'");
